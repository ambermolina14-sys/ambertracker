...
function PollenTab() {
  ...
  const fetchAQI = () => {
    ...
    fetch("https://air-quality-api.open-meteo.com/v1/air-quality?latitude=39.9784&longitude=-86.118&current=us_aqi,pm2_5,pm10,ozone&timezone=America/Indianapolis")
      .then(r => r.json())
      .then(d => {
        setAqiData(d.current);
        setAqiUpdated("Updated " + new Date().toLocaleTimeString());
      })
    ...
    <div>
      ...
      <Card>
        ...
        <div style={{ fontSize: 11, color: "var(--color-text-secondary)", marginTop: ".75rem" }}>
          Based on Indiana seasonal patterns. Granular daily pollen counts are not available for the US via free APIs — this reflects what is typically active in Carmel in {MONTHS[month]}.
        </div>
      </Card>
      ...
      <Card>
        {aqiData === "loading" && <div style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>Fetching air quality data...</div>}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: ".75rem" }}>
              {[
                ...
                <div key={item.label} style={{ background: "var(--color-background-secondary)", borderRadius: 8, padding: ".6rem .75rem" }}>
                  ...
                  {item.isAqi ? <AQIBadge val={item.val} /> : <div style={{ fontSize: 13, fontWeight: 500, color: "var(--color-text-primary)" }}>{item.val}</div>}
                </div>
              ))}
            </div>
      ...
      {/* Body check-in — tap what applies */}
      <SectionTitle>How is your body feeling right now?</SectionTitle>
      <div style={{ fontSize: 12, color: "var(--color-text-secondary)", marginBottom: ".6rem" }}>
        Tap anything that applies — no need to diagnose yourself
      </div>
        ...
        {BODY_SYMPTOMS.map(s => (
          <button key={s.id} onClick={() => toggleTap(s.id)}
            style={{
              ...
              fontWeight: tapped[s.id] ? 500 : 400,
              transition: "all .12s",
            }}>
            {s.icon} {s.label}
          </button>
      ...
      {activeTapped.length > 0 && (
        <div style={{ fontSize: 12, background: colors.blushLight, color: colors.blush, borderRadius: 8, padding: ".6rem .9rem", marginBottom: ".75rem", lineHeight: 1.5 }}>
          Logged: {activeTapped.map(s => s.label).join(", ")}
        </div>
      )}

      <textarea value={notes} onChange={e => setNotes(e.target.value)}
        placeholder="Any other notes — what you ate, where you went, how you feel overall..."
        style={{ width: "100%", minHeight: 70, padding: "8px 10px", fontSize: 13, border: ".5px solid var(--color-border-tertiary)", borderRadius: 8, background: "var(--color-background-primary)", color: "var(--color-text-primary)", fontFamily: "inherit", resize: "vertical", lineHeight: 1.6, boxSizing: "border-box" }} />
      ...
      <Card>
        <div style={{ fontSize: 12, color: "var(--color-text-secondary)", lineHeight: 1.8 }}>
          <div style={{ marginBottom: ".4rem" }}><strong style={{ color: "var(--color-text-primary)", fontWeight: 500 }}>Why you're sensitive:</strong> FCER1A (reactive mast cells) + AOC1 (low DAO enzyme) + MAOB + HNMT variants mean histamine is produced in excess and cleared slowly.</div>
          <div style={{ marginBottom: ".4rem" }}><strong style={{ color: "var(--color-text-primary)", fontWeight: 500 }}>Triggers:</strong> High pollen days, barometric drops, estrogen spikes, high-histamine foods, stress.</div>
          <div><strong style={{ color: "var(--color-text-primary)", fontWeight: 500 }}>Luteal phase rule:</strong> Sardines every other day only. No leftovers. No fermented foods. Estrogen amplifies mast cell reactivity in this phase.</div>
        </div>
      </Card>
    </div>
  );
